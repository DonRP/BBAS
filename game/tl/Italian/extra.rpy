# TODO: Translation updated at 2021-06-19 20:09

translate crowdin strings:

    # game/extra/extra.rpy:56
    old "Новый купальник Лизы"
    new "Il nuovo bikini di Lisa"

    # game/extra/extra.rpy:56
    old "Извинительная пижамка для Алисы"
    new "Pigiama apologetico per Alice"

    # game/extra/extra.rpy:56
    old "Тёмное кружево"
    new "Pizzo scuro"

    # game/extra/extra.rpy:56
    old "Давай я нанесу крем"
    new "Lasciami mettere la crema."

    # game/extra/extra.rpy:56
    old "Ножкам приятно"
    new "I piedi si sentono bene."

    # game/extra/extra.rpy:56
    old "Помассирую не только ножки"
    new "Помассирую не только ножки"

    # game/extra/extra.rpy:56
    old "Первый массаж ног"
    new "Primo massaggio ai piedi"

    # game/extra/extra.rpy:56
    old "Внимание к пальчикам"
    new "Attenzione alle dita"

    # game/extra/extra.rpy:56
    old "Разомнём и плечики"
    new "Разомнём и плечики"

    # game/extra/extra.rpy:56
    old "Массаж для любимой тёти"
    new "Massaggio per la tua zia preferita"

    # game/extra/extra.rpy:56
    old "Совсем другой массаж"
    new "Un massaggio molto diverso"

    # game/extra/extra.rpy:56
    old "Ночные страхи"
    new "Terrori notturni"

    # game/extra/extra.rpy:56
    old "Кто это там ползёт"
    new "Chi è che striscia"

    # game/extra/extra.rpy:56
    old "Монстр в ванной комнате"
    new "Mostro in bagno"

    # game/extra/extra.rpy:56
    old "Держи и не отпускай"
    new "Tieni duro e non mollare"

    # game/extra/extra.rpy:56
    old "После клуба"
    new "Dopo il club"

    # game/extra/extra.rpy:56
    old "Я была плохой девочкой"
    new "Sono stata una ragazza cattiva"

    # game/extra/extra.rpy:56
    old "Я принес тебе полотенце!"
    new "Ti ho portato l'asciugamano!"

    # game/extra/extra.rpy:56
    old "Первые снимки для блога Алисы"
    new "Le prime foto per il blog di Alice"

    # game/extra/extra.rpy:56
    old "Я обошёл Эрика с подарком для Алисы"
    new "Я обошёл Эрика с подарком для Алисы"

    # game/extra/extra.rpy:56
    old "Смотрим порно вместе с тётей"
    new "Guardare il porno con tua zia"

    # game/extra/extra.rpy:56
    old "Первый урок поцелуев"
    new "Prima lezione di bacio"

    # game/extra/extra.rpy:56
    old "А это уже совсем не массаж!"
    new "E questo non è affatto un massaggio!"

    # game/extra/extra.rpy:56
    old "Второй урок поцелуев"
    new "Seconda lezione di bacio"

    # game/extra/extra.rpy:56
    old "Третий урок поцелуев"
    new "Terza lezione di bacio"

    # game/extra/extra.rpy:56
    old "Горячее, чем порно"
    new "Più sexy del porno"

    # game/extra/extra.rpy:56
    old "Небольшое приключение перед сном"
    new "Una piccola avventura prima di andare a letto"

    # game/extra/extra.rpy:56
    old "Не зря купил сорочку"
    new "Ha comprato una camicia per un motivo"

    # game/extra/extra.rpy:56
    old "С меня приятный должок"
    new "Ti devo un bel favore"

    # game/extra/extra.rpy:56
    old "Стриптиз после клуба"
    new "Lo striptease dopo il club"

    # game/extra/extra.rpy:56
    old "Порно-портфолио для Киры"
    new "Portafoglio porno per Kira."

    # game/extra/extra.rpy:56
    old "Немного БДСМ от Киры"
    new "Un po' di BDSM da parte di Kira"

    # game/extra/extra.rpy:56
    old "Вкусные уроки с сестрёнкой"
    new "Lezioni gustose con tua sorella"

    # game/extra/extra.rpy:56
    old "Кажется, мы что-то забыли"
    new "Credo che stiamo dimenticando qualcosa"

    # game/extra/extra.rpy:56
    old "Больше, чем помощь с домашним заданием"
    new "Più di un aiuto per i compiti"

    # game/extra/extra.rpy:56
    old "Ужастики в обнимку с Лизой"
    new "Ужастики в обнимку с Лизой"

    # game/extra/extra.rpy:56
    old "Урок по минету от мамы и Эрика"
    new "Una lezione di pompino da mamma ed Eric"

    # game/extra/extra.rpy:56
    old "Так близко к маминой попке"
    new "Così vicino al culo della mamma"

    # game/extra/extra.rpy:56
    old "Глубокий минет в мамином исполнении"
    new "Un pompino profondo eseguito da mia madre"

    # game/extra/extra.rpy:56
    old "Её первые познания..."
    new "La sua prima conoscenza..."

    # game/extra/extra.rpy:56
    old "Как возбудить ещё больше?"
    new "Come eccitarsi di più?"

    # game/extra/extra.rpy:56
    old "Нежно и аккуратно!"
    new "Bello e ordinato!"

    # game/extra/extra.rpy:56
    old "Как долго это нужно делать?"
    new "Quanto tempo ci vuole per farlo?"

    # game/extra/extra.rpy:56
    old "Кружевное боди для Алисы от Эрика"
    new "Body in pizzo per Alice di Eric"

    # game/extra/extra.rpy:139
    old "ВОСПОМИНАНИЯ"
    new "CONSIGLI"

    # game/extra/extra.rpy:142
    old "ФОТОСНИМКИ"
    new "ФОТОСНИМКИ"

    # game/extra/extra.rpy:193
    old "Воспоминание недоступно"
    new "Ricordo non disponibile"

    # game/extra/extra.rpy:199
    old "Воспоминание ещё не открыто"
    new "Воспоминание ещё не открыто"

    # game/extra/extra.rpy:205
    old "В вашей коллекции ещё нет фотоснимков."
    new "Non hai ancora nessuna foto nella tua collezione."

    # game/extra/extra.rpy:274
    old "Предыдущий снимок"
    new "Foto precedente"

    # game/extra/extra.rpy:280
    old "Вернуться в коллекцию"
    new "Torna alla collezione"

    # game/extra/extra.rpy:285
    old "Следующий снимок"
    new "Prossima foto"

