#     old ""
#     new ""
# "Plural-Forms: nplurals=2; plural=(n != 1);\n"
# "X-Crowdin-Project: big-brother-as\n"
# "X-Crowdin-Project-ID: 461654\n"
# "X-Crowdin-Language: it\n"
# "X-Crowdin-File: /master/game/tl/crowdin/core/10_main_menu.po\n"
# "X-Crowdin-File-ID: 3305\n"
# "Project-Id-Version: big-brother-as\n"
# "Content-Type: text/plain; charset=UTF-8\n"
# "Language-Team: Italian\n"
# "Language: it_IT\n"
# "PO-Revision-Date: 2022-01-26 18:41\n"

# TODO: Translation updated at 2022-01-09 20:33## # translate crowdin strings:#    # game/core/10_main_menu.rpy:51
    old "{color=[lime]}Открыта детальная настройка для одного из костюмов %s в главном меню{/color}"
    new "{color=[lime]}L'impostazione dettagliata di uno dei costumi %s nel menu principale è aperta{/color}"

#    # game/core/10_main_menu.rpy:287
    old "{color=#00FF00}Открыт новый костюм %s для главного меню{/color}"
    new "{color=#00FF00}Nuovo costume %s per il menu principale aperto{/color}"

#    # game/core/10_main_menu.rpy:416
    old "Шорты"
    new "Pantaloncini"

#    # game/core/10_main_menu.rpy:416
    old "Трусы"
    new "Mutandine"

#    # game/core/10_main_menu.rpy:416
    old "Шапка"
    new "Cappello"

#    # game/core/10_main_menu.rpy:416
    old "Борода"
    new "Barba"

#    # game/core/10_main_menu.rpy:416
    old "Трусы #1"
    new "Mutande #1"

#    # game/core/10_main_menu.rpy:416
    old "Трусы #2"
    new "Mutande #2"

#    # game/core/10_main_menu.rpy:416
    old "Маечка"
    new "T-shirt"

#    # game/core/10_main_menu.rpy:416
    old "Минишортики"
    new "Minishorts"

#    # game/core/10_main_menu.rpy:416
    old "Лиф"
    new "Corpetto"

#    # game/core/10_main_menu.rpy:416
    old "Плавки"
    new "Costumi da bagno"

#    # game/core/10_main_menu.rpy:416
    old "Лифчик"
    new "Reggiseno"

#    # game/core/10_main_menu.rpy:416
    old "Трусики"
    new "Mutandine"

#    # game/core/10_main_menu.rpy:416
    old "Шапочка"
    new "Berretta"

#    # game/core/10_main_menu.rpy:416
    old "Бант на шею"
    new "Arco sul collo"

#    # game/core/10_main_menu.rpy:416
    old "Кружевное платье"
    new "Abito in pizzo"

#    # game/core/10_main_menu.rpy:416
    old "Чулочки"
    new "Calze"

#    # game/core/10_main_menu.rpy:416
    old "Топик"
    new "Argomento"

#    # game/core/10_main_menu.rpy:416
    old "Шортики"
    new "Pantaloncini"

#    # game/core/10_main_menu.rpy:416
    old "Шляпка"
    new "Cappello"

#    # game/core/10_main_menu.rpy:416
    old "Бабочка на шею"
    new "Collo a farfalla"

#    # game/core/10_main_menu.rpy:416
    old "Мишура на груди"
    new "Orpelli sul petto"

#    # game/core/10_main_menu.rpy:416
    old "Минилиф"
    new "Minilife"

#    # game/core/10_main_menu.rpy:416
    old "Миниплавки"
    new "Mini-melts"

#    # game/core/10_main_menu.rpy:416
    old "Сорочка"
    new "Camicia"

#    # game/core/10_main_menu.rpy:416
    old "Свитер без рукавов"
    new "Maglione senza maniche"

#    # game/core/10_main_menu.rpy:416
    old "Откровенный корсет"
    new "Candid corsetto"

#    # game/core/10_main_menu.rpy:416
    old "Браслетики на запястья"
    new "Braccialetti sul polso"

#    # game/core/10_main_menu.rpy:416
    old "Юбочка"
    new "Gonna"

#    # game/core/10_main_menu.rpy:416
    old "Штанишки"
    new "Pantaloni"

#    # game/core/10_main_menu.rpy:416
    old "Леденец на шею"
    new "Lecca-lecca sul collo"

#    # game/core/10_main_menu.rpy:416
    old "Свитер с рукавами"
    new "Maglione a maniche lunghe"

#    # game/core/10_main_menu.rpy:416
    old "Колготки"
    new "Collant"

