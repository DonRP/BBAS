msgid ""
msgstr ""
"Plural-Forms: nplurals=2; plural=(n != 1);\n"
"X-Crowdin-Project: big-brother-as\n"
"X-Crowdin-Project-ID: 461654\n"
"X-Crowdin-Language: es-ES\n"
"X-Crowdin-File: /master/game/tl/crowdin/core/04function.po\n"
"X-Crowdin-File-ID: 3430\n"
"Project-Id-Version: big-brother-as\n"
"Content-Type: text/plain; charset=UTF-8\n"
"Language-Team: Spanish\n"
"Language: es_ES\n"
"PO-Revision-Date: 2022-03-27 14:45\n"

# TODO: Translation updated at 2022-03-26 HH:HH# XXtranslate crowdin strings:XX# XX## game/core/04function.rpy:302
msgid "+ к навыку скрытности"
msgid_plural "Ignore this. translate only: One"
msgstr[0] ""
msgstr[1] ""

# XX## game/core/04function.rpy:306
msgid "+ к навыку убеждения"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la habilidad de persuasión"
msgstr[1] ""

# XX## game/core/04function.rpy:310
msgid "+ к навыку массажа"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la habilidad del masaje"
msgstr[1] ""

# XX## game/core/04function.rpy:314
msgid "+ к навыку поцелуев"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la habilidad de besar"
msgstr[1] ""

# XX## game/core/04function.rpy:318
msgid "+ к навыку эротического массажа"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la habilidad del masaje erótico"
msgstr[1] ""

# XX## game/core/04function.rpy:322
msgid "+ к тренированности"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a fitness"
msgstr[1] ""

# XX## game/core/04function.rpy:326
msgid "+ к навыку кунилингуса"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la habilidad de"
msgstr[1] ""

# XX## game/core/04function.rpy:330
msgid "+ к сексуальному опыту"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la experiencia sexual"
msgstr[1] ""

# XX## game/core/04function.rpy:334
msgid "+ к опыту анального секса"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "+ a la experiencia del sexo"
msgstr[1] ""

# XX## game/core/04function.rpy:343
msgid "{color=[red]}значительно ухудшилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[red]}se ha deteriorado significativamente{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:343
msgid "{color=[red]}ухудшилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[red]}empeorado{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:343
msgid "{color=[red]}немного ухудшилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[red]}ligeramente empeorado{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:343
msgid "{color=[lime]}немного улучшилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[lime]}ligeramente mejorado{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:343
msgid "{color=[lime]}улучшилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[lime]}mejorado{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:343
msgid "{color=[lime]}значительно улучшилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[lime]}ha mejorado significativamente{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:354
msgid "{color=[red]}снизилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[red]}disminuido{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:354
msgid "{color=[lime]}повысилось{/color}"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "{color=[lime]}increased{/color}"
msgstr[1] ""

# XX## game/core/04function.rpy:385
msgid "Настроение %s %s \n"
"Её отношение к Максу %s"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Su estado de ánimo %s %s \n"
"su actitud hacia Max %s"
msgstr[1] ""

# XX## game/core/04function.rpy:387
msgid "Отношение %s к Максу %s"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "La actitud de %s hacia Max %s"
msgstr[1] ""

# XX## game/core/04function.rpy:389
msgid "Настроение %s %s"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "%s %sdel estado de ánimo"
msgstr[1] ""

# XX## game/core/04function.rpy:406
msgid "Так... В накладной написано следующее:"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Así que... La factura dice lo siguiente:"
msgstr[1] ""

# XX## game/core/04function.rpy:712
msgid "Прохладные"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Fresco"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Война"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Guerra"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Враждебные"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Hostil"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Плохие"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Malo"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Неплохие"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Bien"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Хорошие"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Bien"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Тёплые"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Cálido"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Дружеские"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Amistoso"
msgstr[1] ""

# XX## game/core/04function.rpy:714
msgid "Близкие"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Cerrar"
msgstr[1] ""

# XX## game/core/04function.rpy:1011
msgid "Конфеты закончились"
msgid_plural "Ignore this. translate only: One"
msgstr[0] "Se acabó el caramelo"
msgstr[1] ""

