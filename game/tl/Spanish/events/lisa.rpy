#     old ""
# "
# "Plural-Forms: nplurals=2; plural=(n != 1);\n"
# "X-spanish-Project: big-brother-as\n"
# "X-spanish-Project-ID: 461654\n"
# "X-spanish-Language: es-ES\n"
# "X-spanish-File: /master/game/tl/spanish/events/lisa.po\n"
# "X-spanish-File-ID: 3488\n"
# "Project-Id-Version: big-brother-as\n"
# "Content-Type: text/plain; charset=UTF-8\n"
# "Language-Team: Spanish\n"
# "Language: es_ES\n"
# "PO-Revision-Date: 2022-03-06 11:18\n"

# TODO: Translation updated at 2022-03-05 HH:HH

# game/events/lisa.rpy:24
translate spanish lisa_shower_4cc15313:

    # Max_00 "{m}Лиза сейчас принимает душ...{/m}" nointeract

# game/events/lisa.rpy:28
translate spanish lisa_shower_bef35b29:

    # Max_14 "{m}Лиза уже поймала меня на подглядывании. Грозилась рассказать маме. Не стоит злить её ещё больше.{/m}"

# game/events/lisa.rpy:31
translate spanish lisa_shower_e5bfe1e2:

    # Max_09 "{m}Сегодня я уже чуть не попался Лизе при подглядывании. Повезло, что успел вовремя сбежать. Не стоит рисковать ещё раз.{/m}"

# game/events/lisa.rpy:34
translate spanish lisa_shower_1e033770:

    # Max_01 "{m}Сегодня я уже подсматривал за Лизой. Повезло, что она меня не заметила. Не стоит рисковать ещё раз.{/m}"

# game/events/lisa.rpy:42
translate spanish lisa_shower_21893e03:

    # Max_09 "{m}Кажется, Лиза что-то делает в ванной...{/m}" nointeract

# game/events/lisa.rpy:45
translate spanish lisa_shower_7a2afa4f:

    # Lisa "{b}Лиза:{/b} Кто там? Я ещё не закончила. Подождите немного..." nointeract
    Lisa "{b}Lisa:{/b} ¿Quién está ahí? No he terminado. Espera un poco..." nointeract

# game/events/lisa.rpy:48
translate spanish lisa_shower_0451df43:

    # Lisa "{b}Лиза:{/b} Макс, чего хотел? Я же говорю, скоро выйду!" nointeract
    Lisa "{b}Lisa:{/b} Max, ¿qué querías? Digo, ¡saldré pronto!" nointeract

# game/events/lisa.rpy:51
translate spanish lisa_shower_d84f6a90:

    # Lisa "{b}Лиза:{/b} Нет, Макс. Жди за дверью. Я скоро!" nointeract
    Lisa " {b} Lisa: {/ b} No, Max. Espera fuera de la puerta. ¡Estaré allí pronto! " nointeract

# game/events/lisa.rpy:63
translate spanish lisa_shower_44635170:

    # Max_10 "{m}Хочется, конечно, ещё разок взглянуть на голую сестрёнку, но я ещё не отдал ей обещанное...{/m}"

# game/events/lisa.rpy:76
translate spanish lisa_shower_ladder_76e8b72b:

    # Max_04 "{m}Посмотрим, что у нас тут...{/m}"

# game/events/lisa.rpy:91
translate spanish lisa_shower_ladder_370767aa:

    # Max_03 "{m}Класс! Лиза смотрится в подаренном мною халатике очень соблазнительно... Особенно когда так хорошо видно её упругие сисечки!{/m}"

# game/events/lisa.rpy:93
translate spanish lisa_shower_ladder_d5ff7920:

    # Max_07 "{m}О, да! Моя обворожительная сестрёнка в одних трусиках... Так и хочется зайти и стянуть их с её прекрасной попки!{/m}"

# game/events/lisa.rpy:95
translate spanish lisa_shower_ladder_d17edcf4:

    # Max_06 "{m}Ого! Утро может быть действительно очень добрым, если удаётся полюбоваться совершенно голенькой Лизой! Да... её тело завораживает...{/m}"

# game/events/lisa.rpy:100
translate spanish lisa_shower_ladder_1409147d:

    # Max_07 "{m}Мои зрители явно пропускают много всего интересного! Мне однозначно стоит установить сюда ещё одну камеру...{/m}"

# game/events/lisa.rpy:101
translate spanish lisa_shower_ladder_63c0b6bb:

    # Max_00 "{m}Хоть и не хочется, но пока меня не заметили, лучше уходить...{/m}"

# game/events/lisa.rpy:113
translate spanish lisa_shower_start_peeping_fc5db481:

    # Max_09 "{m}Лучше вообще свести подглядывания к минимуму, пока я не избавлюсь от Эрика. Чтобы никого ещё больше не расстраивать...{/m}"

# game/events/lisa.rpy:115
translate spanish lisa_shower_start_peeping_dda24c4d:

    # Max_01 "{m}Хорошо, что это не распространяется на Киру...{/m}" nointeract

# game/events/lisa.rpy:122
translate spanish lisa_shower_start_peeping_babc130b:

    # Max_07 "{m}Отлично! Моя младшая сестрёнка принимает душ... Даже видно кое-что... Много кое-чего! Только бы она меня не заметила...{/m}" nointeract

# game/events/lisa.rpy:146
translate spanish lisa_shower_alt_peepeng_8578463f:

    # Max_02 "[undetect!t]{m}Лиза вся такая мокренькая... класс! Фигурка и всё остальное у неё – что надо... Как же хочется потрогать!{/m}"

# game/events/lisa.rpy:148
translate spanish lisa_shower_alt_peepeng_cd01369c:

    # Max_03 "[undetect!t]{m}О, да! За тем, как вода стекает по её обворожительной попке, хочется смотреть не отрываясь...{/m}"

# game/events/lisa.rpy:161
translate spanish lisa_shower_closer_peepeng_8578463f:

    # Max_02 "[undetect!t]{m}Лиза вся такая мокренькая... класс! Фигурка и всё остальное у неё – что надо... Как же хочется потрогать!{/m}"

# game/events/lisa.rpy:163
translate spanish lisa_shower_closer_peepeng_cd01369c:

    # Max_03 "[undetect!t]{m}О, да! За тем, как вода стекает по её обворожительной попке, хочется смотреть не отрываясь...{/m}"

# game/events/lisa.rpy:178
translate spanish lisa_shower_not_luck_76384340:

    # Max_12 "{color=[orange]}{i}Кажется, Лиза что-то заподозрила!{/i}{/color}\n{m}О нет! Похоже, она что-то заметила... Надо бежать!{/m}"

# game/events/lisa.rpy:200
translate spanish lisa_shower_pinded_aded2cb3:

    # Lisa_11 "[spotted!t]Ой, Макс! Опять ты подглядываешь... Это уже маньячество какое-то!"

# game/events/lisa.rpy:201
translate spanish lisa_shower_pinded_839bdc8e:

    # Max_02 "Просто любуюсь формами! А так, я вообще мимо шёл, а здесь ты..."

# game/events/lisa.rpy:202
translate spanish lisa_shower_pinded_c430f032:

    # Lisa_10 "Вот и иди мимо! И без майки ты меня теперь две недели точно не увидишь. Помнишь, я предупреждала?"

# game/events/lisa.rpy:203
translate spanish lisa_shower_pinded_02f58c38:

    # Max_08 "Блин! Точно, вспомнил... Но, может..."

# game/events/lisa.rpy:204
translate spanish lisa_shower_pinded_d7baed79:

    # Lisa_13 "Нет. Это будет для тебя уроком!"

# game/events/lisa.rpy:205
translate spanish lisa_shower_pinded_7428c407:

    # Max_11 "Ладно..."

# game/events/lisa.rpy:210
translate spanish lisa_shower_pinded_cc880acd:

    # Lisa_09 "[spotted!t]Ну, Макс! Опять ты подглядываешь... Если так неймётся ужастики смотреть со мной, то считай ты попал! А сейчас, кыш отсюда..." nointeract

# game/events/lisa.rpy:215
translate spanish lisa_shower_pinded_e7c30df4:

    # Lisa_13 "[spotted!t]Ай, Макс!!! Ты в конец бессовестный что ли?! Отвернись немедленно и иди куда шёл!"

# game/events/lisa.rpy:216
translate spanish lisa_shower_pinded_6f1ab50c:

    # Max_13 "Лиза, я случайно! Ты же не расскажешь маме?"

# game/events/lisa.rpy:217
translate spanish lisa_shower_pinded_1ad82afe:

    # Lisa_14 "Ещё как расскажу! Сразу же, как только душ спокойно приму... А ты, вали с глаз моих!"

# game/events/lisa.rpy:218
translate spanish lisa_shower_pinded_e472503c:

    # Max_10 "Вот же попал!"

# game/events/lisa.rpy:225
translate spanish lisa_shower_pinded_05cff630:

    # Lisa_12 "[spotted!t]Макс! Ты подглядываешь за мной? Как тебе не стыдно?! Я всё маме расскажу!" nointeract

# game/events/lisa.rpy:270
translate spanish lisa_dressed_69f4873f:

    # Max_09 "{m}Сейчас Лиза должна собираться в школу...{/m}" nointeract

# game/events/lisa.rpy:273
translate spanish lisa_dressed_dad74527:

    # Max_09 "{m}Сейчас Лиза должна переодеваться, чтобы отправиться вместе со всеми по магазинам...{/m}" nointeract

# game/events/lisa.rpy:276
translate spanish lisa_dressed_aaeb2a78:

    # Max_09 "{m}Сейчас Лиза обычно переодевается, чтобы отправиться на занятие к репетитору...{/m}" nointeract

# game/events/lisa.rpy:279
translate spanish lisa_dressed_c83f0902:

    # Max_09 "{m}Сейчас Лиза обычно переодевается, чтобы пойти погулять. Ну, или что она там ещё делает...{/m}" nointeract

# game/events/lisa.rpy:293
translate spanish lisa_dressed_869e0bce:

    # Lisa "{b}Лиза:{/b} Кто там? Я переодеваюсь!" nointeract

# game/events/lisa.rpy:299
translate spanish lisa_dressed_3c9093ff:

    # Lisa "{b}Лиза:{/b} Макс, дай одеться спокойно! Потом свой ноутбук заберёшь..." nointeract

# game/events/lisa.rpy:328
translate spanish lisa_dressed_ad44f284:

    # Max_01 "{m}Ого, какой вид! Вот это я удачно заглянул!{/m}" nointeract

# game/events/lisa.rpy:343
translate spanish lisa_dressed_get_notebook0_02a8d3e8:

    # Lisa_11 "Макс! Я не одета!!! Быстро закрой дверь с той стороны и не входи, пока я не переоденусь!"

# game/events/lisa.rpy:344
translate spanish lisa_dressed_get_notebook0_43ba5f09:

    # Max_09 "Лиза, мне ноутбук нужен для дела!"

# game/events/lisa.rpy:345
translate spanish lisa_dressed_get_notebook0_4d87ccb7:

    # Lisa_13 "Какого дела? Ты дома сидишь целыми днями и ничего не делаешь..."

# game/events/lisa.rpy:346
translate spanish lisa_dressed_get_notebook0_0677ddc2:

    # Lisa_12 "Ладно, неважно... Забирай свой ноутбук и уходи. Дай мне уже переодеться..." nointeract

# game/events/lisa.rpy:348
translate spanish lisa_dressed_get_notebook0_fb4e2ab0:

    # Lisa_11 "Макс! Я не одета!!! Выйди и закрой за собой дверь! Мне нужно переодеться!"

# game/events/lisa.rpy:349
translate spanish lisa_dressed_get_notebook0_43ba5f09_1:

    # Max_09 "Лиза, мне ноутбук нужен для дела!"

# game/events/lisa.rpy:350
translate spanish lisa_dressed_get_notebook0_d66e147c:

    # Lisa_09 "Деловой, куда деваться! Сказал бы хоть, что заходишь..."

# game/events/lisa.rpy:351
translate spanish lisa_dressed_get_notebook0_2d30e546:

    # Lisa_10 "Ты не глазей, а ноутбук быстрее забирай и уходи. Дай мне уже переодеться..." nointeract

# game/events/lisa.rpy:353
translate spanish lisa_dressed_get_notebook0_6588bce8:

    # Lisa_11 "Макс! Я не одета!!! Выйди, пожалуйста, чтобы я могла переодеться! И дверь за собой закрой!"

# game/events/lisa.rpy:354
translate spanish lisa_dressed_get_notebook0_43ba5f09_2:

    # Max_09 "Лиза, мне ноутбук нужен для дела!"

# game/events/lisa.rpy:357
translate spanish lisa_dressed_get_notebook0_d66e147c_1:

    # Lisa_09 "Деловой, куда деваться! Сказал бы хоть, что заходишь..."

# game/events/lisa.rpy:358
translate spanish lisa_dressed_get_notebook0_2d30e546_1:

    # Lisa_10 "Ты не глазей, а ноутбук быстрее забирай и уходи. Дай мне уже переодеться..." nointeract

# game/events/lisa.rpy:361
translate spanish lisa_dressed_get_notebook0_eb706326:

    # Lisa_09 "Не раньше тебе и не позже! Бери и уходи. Я хочу спокойно переодеться..." nointeract

# game/events/lisa.rpy:372
translate spanish lisa_dressed_get_notebook1_657f8291:

    # Lisa "{b}Лиза:{/b} Ладно, так уж и быть, забирай быстрее..." nointeract

# game/events/lisa.rpy:375
translate spanish lisa_dressed_get_notebook1_77805a20:

    # Lisa_01 "Ты что, без ноутбука и часа прожить не можешь? Подождать никак не мог?"

# game/events/lisa.rpy:376
translate spanish lisa_dressed_get_notebook1_968d6553:

    # Max_07 "Нет, он мне очень срочно нужен для дела!"

# game/events/lisa.rpy:377
translate spanish lisa_dressed_get_notebook1_6388868d:

    # Lisa_02 "Сомневаюсь, что у тебя там могут быть какие-то сверхважные дела..."

# game/events/lisa.rpy:378
translate spanish lisa_dressed_get_notebook1_8649258d:

    # Lisa_00 "Ладно, неважно... Дай мне уже переодеться!" nointeract

# game/events/lisa.rpy:380
translate spanish lisa_dressed_get_notebook1_a85f1f9c:

    # Lisa_01 "Вот именно сейчас он тебе обязательно понадобился, да? Подождать никак не мог?"

# game/events/lisa.rpy:381
translate spanish lisa_dressed_get_notebook1_968d6553_1:

    # Max_07 "Нет, он мне очень срочно нужен для дела!"

# game/events/lisa.rpy:382
translate spanish lisa_dressed_get_notebook1_07b6f868:

    # Lisa_02 "Макс, если под \"делами\" ты подразумеваешь какие-нибудь детские передачи, то пора бы уже из этого вырасти..."

# game/events/lisa.rpy:383
translate spanish lisa_dressed_get_notebook1_8649258d_1:

    # Lisa_00 "Ладно, неважно... Дай мне уже переодеться!" nointeract

# game/events/lisa.rpy:386
translate spanish lisa_dressed_get_notebook1_a6bb39c7:

    # Lisa_01 "Хорошо, что я уже почти оделась. Или ноутбук - это просто предлог и ты надеялся, что я тут полуголая буду?"

# game/events/lisa.rpy:388
translate spanish lisa_dressed_get_notebook1_9ec77846:

    # Lisa_01 "Повезло тебе, что я ещё не начала раздеваться. Или ноутбук - это просто предлог и ты надеялся, что я тут полуголая буду?"

# game/events/lisa.rpy:389
translate spanish lisa_dressed_get_notebook1_32f69930:

    # Max_02 "Не без этого, но он мне действительно очень нужен для дела!"

# game/events/lisa.rpy:390
translate spanish lisa_dressed_get_notebook1_6be31d8d:

    # Lisa_02 "Ага, так я тебе и поверила. Забирай его уже быстрее, деловой..." nointeract

# game/events/lisa.rpy:404
translate spanish lisa_dressed_moment0_e9b34443:

    # Lisa_00 "Ой, Макс! А ты можешь немного погулять? Я переодеться хотела..." nointeract

# game/events/lisa.rpy:407
translate spanish lisa_dressed_moment0_d06bbd27:

    # Lisa_10 "Ну да... Я стесняюсь это делать при тебе. Ты же наверняка подглядывать будешь!" nointeract

# game/events/lisa.rpy:409
translate spanish lisa_dressed_moment0_ca4f8bf0:

    # Lisa_13 "В смысле?! Это когда ты успел меня голой увидеть?" nointeract

# game/events/lisa.rpy:412
translate spanish lisa_dressed_moment0_3e816722:

    # Lisa_09 "Ага! Так я тебе и поверила. Пожалуйста, дай спокойно переодеться. Я же недолго, Макс!" nointeract

# game/events/lisa.rpy:415
translate spanish lisa_dressed_moment0_89f1263a:

    # Lisa_09 "Откуда я знаю, что ты видел, а что ещё нет? Как бы там ни было, я стесняюсь и хочу переодеться без свидетелей." nointeract

# game/events/lisa.rpy:418
translate spanish lisa_dressed_moment0_930f8d87:

    # Lisa_02 "Вот представь, ну никак не могу! Стесняюсь. А переодеться надо. Так что, можно я спокойно это сделаю? И без свидетелей..." nointeract

# game/events/lisa.rpy:422
translate spanish lisa_dressed_moment0_a9de2a0b:

    # Lisa_01 "И сдался он тебе прямо сейчас? Я же не буду одеваться целый час." nointeract

# game/events/lisa.rpy:424
translate spanish lisa_dressed_moment0_ce7b5650:

    # Lisa_01 "Ну забирай, если он тебе так нужен. А так, я недолго буду одеваться, можешь подождать..." nointeract

# game/events/lisa.rpy:429
translate spanish lisa_dressed_moment0_4671244c:

    # Lisa_01 "Спасибо, Макс. Я быстренько!" nointeract

# game/events/lisa.rpy:442
translate spanish lisa_dressed_stay_in_room_4d0209ac:

    # Lisa_09 "Макс, мне нужно переодеться. Будь добр, выйди из комнаты ненадолго..." nointeract

# game/events/lisa.rpy:444
translate spanish lisa_dressed_stay_in_room_9edd3eac:

    # Lisa_00 "Макс, мне нужно переодеться. Будь добр, погуляй немного или отвернись..." nointeract

# game/events/lisa.rpy:450
translate spanish lisa_dressed_stay_in_room_769644c1:

    # Lisa "{b}Лиза:{/b} Я быстро... Главное, не подглядывай! Надеюсь, тебе это по силам..." nointeract

# game/events/lisa.rpy:452
translate spanish lisa_dressed_stay_in_room_08276951:

    # Lisa "{b}Лиза:{/b} И не вздумай подглядывать! Это не займёт много времени..." nointeract

# game/events/lisa.rpy:454
translate spanish lisa_dressed_stay_in_room_cdba7705:

    # Lisa "{b}Лиза:{/b} И я бы хотела, чтобы ты постарался не подглядывать... Знаю, сложно, но пожалуйста!" nointeract

# game/events/lisa.rpy:463
translate spanish lisa_dressed_stay_in_room_db51f185:

    # Max_09 "{m}Я слишком много раз пытался подсмотреть, как Лиза переодевается. Больше не стоит пытаться сделать это сегодня.{/m}"

# game/events/lisa.rpy:482
translate spanish lisa_dressed_moment2_8183fb49:

    # Lisa_11 "Макс! Я не одета!!! {p=3}{nw}"

# game/events/lisa.rpy:488
translate spanish lisa_dressed_moment2_0caabb4b:

    # Lisa_14 "Быстро закрой дверь с той стороны и не входи, пока я не переоденусь!" nointeract

# game/events/lisa.rpy:491
translate spanish lisa_dressed_moment2_b3661128:

    # Lisa_13 "Выйди и закрой за собой дверь! Мне нужно переодеться!" nointeract

# game/events/lisa.rpy:494
translate spanish lisa_dressed_moment2_06742870:

    # Lisa_10 "Выйди, пожалуйста, чтобы я могла переодеться! И дверь за собой закрой!" nointeract

# game/events/lisa.rpy:506
translate spanish lisa_dressed_moment1_ab3e5dbc:

    # Lisa_11 "Макс! Я не одета!!! Быстро закрой дверь с той стороны и не входи, пока я не переоденусь!" nointeract

# game/events/lisa.rpy:509
translate spanish lisa_dressed_moment1_f9a4cf77:

    # Lisa_11 "Макс! Я не одета!!! Выйди и закрой за собой дверь! Мне нужно переодеться!" nointeract

# game/events/lisa.rpy:512
translate spanish lisa_dressed_moment1_bf8db93b:

    # Lisa_11 "Макс! Я не одета!!! Выйди, пожалуйста, чтобы я могла переодеться! И дверь за собой закрой!" nointeract

# game/events/lisa.rpy:519
translate spanish lisa_dressed_lvl_1_b2616dcb:

    # Lisa_12 "Что?! Если не уйдёшь, то я всё маме расскажу!" nointeract

# game/events/lisa.rpy:522
translate spanish lisa_dressed_lvl_1_b2616dcb_1:

    # Lisa_12 "Что?! Если не уйдёшь, то я всё маме расскажу!" nointeract

# game/events/lisa.rpy:524
translate spanish lisa_dressed_lvl_1_10a5c581:

    # Lisa_12 "Макс! Я ведь маме на тебя нажалуюсь, если не уйдёшь!" nointeract

# game/events/lisa.rpy:526
translate spanish lisa_dressed_lvl_1_4d33062b:

    # Lisa_13 "Ну и долго ты ещё пялиться на меня будешь?! Выйди и подожди за дверью. Пожалуйста!"

# game/events/lisa.rpy:532
translate spanish lisa_dressed_lvl_2_366df3c4:

    # Lisa_10 "Не надо меня тут комплиментами одаривать, чтобы подольше поглазеть! Или хочешь, чтобы я маме об этом рассказала?" nointeract

# game/events/lisa.rpy:534
translate spanish lisa_dressed_lvl_2_0fe7cd3f:

    # Lisa_10 "А вот не надо на неё так глазеть! Я ведь и маме всё могу рассказать, если не уйдёшь." nointeract

# game/events/lisa.rpy:536
translate spanish lisa_dressed_lvl_2_0d3512aa:

    # Lisa_10 "Ну, Макс! Дай переодеться спокойно... Пожалуйста! Или мне маме пожаловаться?" nointeract

# game/events/lisa.rpy:540
translate spanish lisa_dressed_lvl_2_766601ad:

    # Lisa_09 "Ну и долго ты ещё пялиться на меня будешь?! Выйди и подожди. Я недолго..." nointeract

# game/events/lisa.rpy:547
translate spanish lisa_dressed_lvl_3_9f4e5577:

    # Lisa_01 "Ты, видимо, только об этом и мечтаешь... Но нечего глазеть, как я одеваюсь! Лучше займи себя чем-нибудь полезным..."

# game/events/lisa.rpy:548
translate spanish lisa_dressed_lvl_3_f073ff95:

    # Max_02 "Как раз это и делаю..."

# game/events/lisa.rpy:549
translate spanish lisa_dressed_lvl_3_01edfdc0:

    # Lisa_09 "Ну, Макс!" nointeract

# game/events/lisa.rpy:552
translate spanish lisa_dressed_lvl_3_53bb992b:

    # Lisa_01 "Думаешь, она у меня такая, чтобы тебя постоянно радовать? А вот и нет! Дай одеться спокойно..."

# game/events/lisa.rpy:553
translate spanish lisa_dressed_lvl_3_f331a21c:

    # Max_02 "Ага, сейчас. Только ещё немного полюбуюсь..."

# game/events/lisa.rpy:554
translate spanish lisa_dressed_lvl_3_01edfdc0_1:

    # Lisa_09 "Ну, Макс!" nointeract

# game/events/lisa.rpy:556
translate spanish lisa_dressed_lvl_3_aa58b1e2:

    # Lisa_01 "Ну ничего, Макс, ничего... Я скоро уйду и ты сможешь вдоволь поплакать. Сильно расстроился?"

# game/events/lisa.rpy:557
translate spanish lisa_dressed_lvl_3_8cdcdb2c:

    # Max_02 "Нет. Как представляю, что я тут пропустил - сразу приятно становится."

# game/events/lisa.rpy:558
translate spanish lisa_dressed_lvl_3_ab614b1d:

    # Lisa_09 "Ну, Макс!"

# game/events/lisa.rpy:560
translate spanish lisa_dressed_lvl_3_1e3411b7:

    # Lisa_13 "Ты! Ничего! Не видел! Понял? И хватит на меня глазеть с таким довольным лицом... Выйди и подожди за дверью!" nointeract

# game/events/lisa.rpy:564
translate spanish lisa_dressed_lvl_3_fd7a8374:

    # Lisa_01 "Спасибо. Но давай ты не будешь пялиться и подождёшь за дверью, я недолго... Хорошо?"

# game/events/lisa.rpy:565
translate spanish lisa_dressed_lvl_3_fb1e1d49:

    # Max_04 "А может повторим? Мне понравилось..."

# game/events/lisa.rpy:566
translate spanish lisa_dressed_lvl_3_b98d244b:

    # Lisa_02 "Макс, займись чем-нибудь путным." nointeract

# game/events/lisa.rpy:577
translate spanish lisa_dressed_try_to_peek_b3342fcc:

    # Max_05 "[lucky!t]{m}Повезло! Лиза не видит, что я подглядываю... Ещё совсем юная, но уже такая сексуальная! Обожаю её...{/m}"

# game/events/lisa.rpy:579
translate spanish lisa_dressed_try_to_peek_274e1029:

    # Max_05 "[lucky!t]{m}Класс! Кажется, она не видит, что я подглядываю... Очень сложно оторвать взгляд от этих соблазнительных изгибов её фигуры, но надо...{/m}"

# game/events/lisa.rpy:581
translate spanish lisa_dressed_try_to_peek_6614903d:

    # Max_05 "[lucky!t]{m}Моя сестрёнка такая сладенькая! Как же мне повезло жить с такой классной девочкой в одной комнате и каждый день наслаждаться её голыми прелестями...{/m}"

# game/events/lisa.rpy:590
translate spanish lisa_dressed_try_to_peek_1ae137e9:

    # Lisa_11 "[spotted!t]Макс! Я же просила не подглядывать! Ну-ка быстро отвернись... Ещё раз так сделаешь и я всё маме расскажу! Понял?!" nointeract

# game/events/lisa.rpy:593
translate spanish lisa_dressed_try_to_peek_b99743fe:

    # Lisa_11 "[spotted!t]Ну, Макс! Прекращай подглядывать! Если не отвернёшься или ещё раз такое выкинешь, я на тебя маме нажалуюсь... Тебе ясно?!" nointeract

# game/events/lisa.rpy:595
translate spanish lisa_dressed_try_to_peek_2ba94453:

    # Lisa_11 "[spotted!t]Опять ты подглядываешь! И не стыдно тебе?! Делать тебе больше нечего, только глазеть, как я переодеваюсь... Прекращай!" nointeract

# game/events/lisa.rpy:598
translate spanish lisa_dressed_try_to_peek_d37ef64c:

    # Lisa_12 "Подумаешь, я сильно на тебя обижусь и больше разговаривать не буду..."

# game/events/lisa.rpy:599
translate spanish lisa_dressed_try_to_peek_ff32bf71:

    # Max_08 "Всё, я отвернулся!"

# game/events/lisa.rpy:601
translate spanish lisa_dressed_try_to_peek_e97eac0d:

    # Lisa_10 "А я вот стесняюсь! И перестань на меня так глазеть! А то я Алису попрошу, чтобы она тебя отпинала..."

# game/events/lisa.rpy:602
translate spanish lisa_dressed_try_to_peek_ff32bf71_1:

    # Max_08 "Всё, я отвернулся!"

# game/events/lisa.rpy:606
translate spanish lisa_dressed_try_to_peek_470a2396:

    # Lisa_13 "Ну и долго ты ещё пялиться на меня будешь?! Отвернись!" nointeract

# game/events/lisa.rpy:608
translate spanish lisa_dressed_try_to_peek_f94f03ae:

    # Lisa_09 "Ну и долго ты ещё пялиться на меня будешь?! Отвернись. Я недолго..." nointeract

# game/events/lisa.rpy:613
translate spanish lisa_dressed_try_to_peek_66f1c3a5:

    # Lisa_02 "Вот представь, ну никак не могу! Стесняюсь. Можно я уже спокойно переоденусь? Без этого твоего сверлящего и озабоченного взгляда..."

# game/events/lisa.rpy:614
translate spanish lisa_dressed_try_to_peek_1a1c2375:

    # Max_02 "Можно. Только постой так ещё немного, я быстренько сфотографирую..."

# game/events/lisa.rpy:615
translate spanish lisa_dressed_try_to_peek_ab614b1d:

    # Lisa_09 "Ну, Макс!"

# game/events/lisa.rpy:616
translate spanish lisa_dressed_try_to_peek_2c3cf9b7:

    # Max_03 "Да шучу я... Всё, отвернулся!"

# game/events/lisa.rpy:618
translate spanish lisa_dressed_try_to_peek_e11ee97e:

    # Lisa_01 "Спасибо. Но давай ты не будешь пялиться и отвернёшься... Хорошо?"

# game/events/lisa.rpy:619
translate spanish lisa_dressed_try_to_peek_e22e7d62:

    # Max_04 "Мне понравилось, как ты двигалась... Не повторишь?"

# game/events/lisa.rpy:621
translate spanish lisa_dressed_try_to_peek_01a9904e:

    # Lisa_02 "Макс, ты меня совсем сейчас засмущаешь! Отвернись, пожалуйста..." nointeract

# game/events/lisa.rpy:757
translate spanish lisa_bath_2d4c2fc0:

    # Max_00 "{m}В это время Лиза обычно плескается в ванне...{/m}" nointeract

# game/events/lisa.rpy:765
translate spanish lisa_bath_19da9af2:

    # Max_00 "{m}Кажется, Лиза и правда принимает ванну. Жаль, что из-за матового стекла почти ничего не видно. Но ближе подойти опасно - может заметить...{/m}"

# game/events/lisa.rpy:766
translate spanish lisa_bath_ad0a8c77:

    # Max_09 "{m}Нужно что-нибудь придумать...{/m}"

# game/events/lisa.rpy:772
translate spanish lisa_bath_4a929722:

    # Max_01 "{m}Надеюсь, что ни у кого не возникнет вопроса, а что же здесь делает стремянка... Как, что? Конечно стоит, мало ли что! А теперь начинается самое интересное...{/m}"

# game/events/lisa.rpy:784
translate spanish lisa_bath_ladder_76e8b72b:

    # Max_04 "{m}Посмотрим, что у нас тут...{/m}"

# game/events/lisa.rpy:795
translate spanish lisa_bath_ladder_134cfe35:

    # Max_03 "{m}Кажется, Лиза как раз собирается принять ванну... О да, моя младшая сестрёнка хороша... а голенькая, так особенно!{/m}" nointeract

# game/events/lisa.rpy:801
translate spanish lisa_bath_ladder_3fb74fbf:

    # Max_05 "{m}Ох, вот это повезло! Лиза демонстрирует свои прелестные сисечки словно специально! Разумеется, она не знает, что я смотрю, а то крику бы было...{/m}" nointeract

# game/events/lisa.rpy:810
translate spanish lisa_bath_ladder_3fb74fbf_1:

    # Max_05 "{m}Ох, вот это повезло! Лиза демонстрирует свои прелестные сисечки словно специально! Разумеется, она не знает, что я смотрю, а то крику бы было...{/m}" nointeract

# game/events/lisa.rpy:816
translate spanish lisa_bath_ladder_bedd90b4:

    # Max_07 "{m}Эх! Вот и закончились водные процедуры... Ухх... И с этой обворожительной киской я живу в одной комнате! Красота...{/m}" nointeract

# game/events/lisa.rpy:825
translate spanish lisa_bath_knock_b1c84f9a:

    # Lisa "{b}Лиза:{/b} Кто там? Я принимаю ванну..." nointeract

# game/events/lisa.rpy:828
translate spanish lisa_bath_knock_d71a1367:

    # Lisa "{b}Лиза:{/b} Я же сказала, что в ванне. Закончу, тогда и войдёшь! А пока жди..." nointeract

# game/events/lisa.rpy:846
translate spanish lisa_bath_open_knock_397f3e74:

    # Lisa_11 "Макс! Я же предупредила, что моюсь! Всё маме расскажу!"

# game/events/lisa.rpy:850
translate spanish lisa_bath_open_knock_397f3e74_1:

    # Lisa_11 "Макс! Я же предупредила, что моюсь! Всё маме расскажу!"

# game/events/lisa.rpy:854
translate spanish lisa_bath_open_knock_ebe1fe1e:

    # Lisa_11 "Макс! Я же предупредила, что моюсь! Хочешь со мной поссориться?"

# game/events/lisa.rpy:855
translate spanish lisa_bath_open_knock_e5b17df4:

    # Max_00 "Упс! Уже ушёл..."

# game/events/lisa.rpy:874
translate spanish lisa_bath_open_44974c8f:

    # Lisa_11 "Макс! А постучаться? Я же голая!" nointeract

# game/events/lisa.rpy:881
translate spanish lisa_bath_open_bb4cd26f:

    # Lisa_12 "Макс, выйди немедленно!"

# game/events/lisa.rpy:882
translate spanish lisa_bath_open_3db87e42:

    # Max_00 "Хорошо, уже ухожу..."

# game/events/lisa.rpy:929
translate spanish lisa_romantic_movie_0_f3247db8:

    # Lisa_01 "Ну что, Макс, смотрим кино или как?"
    Lisa_01 "Bueno, Max, ¿ver una película o qué?"

# game/events/lisa.rpy:930
translate spanish lisa_romantic_movie_0_a59bad84:

    # Max_01 "Да, смотрим. Сейчас всё подготовлю..."
    Max_01 "sí, veamos. Prepararé todo ahora..."

# game/events/lisa.rpy:931
translate spanish lisa_romantic_movie_0_8ae1d92e:

    # Lisa_02 "А я пока свет выключу."
    Lisa_02 "Y apagaré las luces por ahora."

# game/events/lisa.rpy:936
translate spanish lisa_romantic_movie_0_16a5711d:

    # Max_04 "Давай, запрыгивай! Ты уже знаешь, что будем смотреть?"

# game/events/lisa.rpy:937
translate spanish lisa_romantic_movie_0_ea5cc644:

    # Lisa_09 "В смысле, \"запрыгивай\"? К тебе на кровать, что ли?"

# game/events/lisa.rpy:938
translate spanish lisa_romantic_movie_0_c52165ea:

    # Max_07 "Да, ко мне. Или к тебе, если хочешь."

# game/events/lisa.rpy:939
translate spanish lisa_romantic_movie_0_dc0d9e48:

    # Lisa_10 "Я думала каждый со своей кровати будет смотреть!"

# game/events/lisa.rpy:940
translate spanish lisa_romantic_movie_0_907e98f3:

    # Max_09 "Здесь же у нас не такой экран, как в гостиной. Смотреть надо близко."

# game/events/lisa.rpy:941
translate spanish lisa_romantic_movie_0_57a4b7fa:

    # Lisa_00 "Ну... ладно. Подвинься тогда."
    Lisa_00 " Bueno... está bien. Muévete entonces."

# game/events/lisa.rpy:945
translate spanish lisa_romantic_movie_0_8a7e7885:

    # Max_00 "Ну так... каким фильмом ты собиралась меня мучить?"
    Max_00 " Entonces... ¿con qué película me ibas a torturar?"

# game/events/lisa.rpy:946
translate spanish lisa_romantic_movie_0_ae7209b1:

    # Lisa_01 "Точно не знаю. Напиши в поиске \"лучшие романтические фильмы\" и я что-нибудь выберу."
    Lisa_01 " No lo sé exactamente. Busque \"mejores películas románticas\" y elegiré algo."

# game/events/lisa.rpy:947
translate spanish lisa_romantic_movie_0_d14c257e:

    # Max_03 "Вот, смотри... Выбирай... Может вот этот? Постер уж очень интересный!"
    Max_03 " Mira... Elige... ¿Quizás este? ¡El cartel es muy interesante!"

# game/events/lisa.rpy:948
translate spanish lisa_romantic_movie_0_8a55ebad:

    # Lisa_02 "Нет, мы будем смотреть то, что интересно мне! Хочу вон тот фильм! Давай, включай..."
    Lisa_02 "¡No, estaremos viendo lo que es interesante para mí! ¡Quiero esa película! Vamos, enciende..."

# game/events/lisa.rpy:950
translate spanish lisa_romantic_movie_0_51f733a8:

    # Max_07 "{m}Да уж, это конечно намного лучше, чем получать при всех от мамы по заднице, но так скучно! Хотя бы с сестрёнкой рядом на одной кровати полежу. А смотреть можно и вполглаза...{/m}"
    Max_07 " {m} Sí, ciertamente es mucho mejor que poner el trasero de tu mamá frente a todos, ¡pero es tan aburrido! Al menos me acostaré en la misma cama con mi hermana pequeña. Y puedes mirar con medio ojo... {/ m}"

# game/events/lisa.rpy:951
translate spanish lisa_romantic_movie_0_60738184:

    # Lisa_13 "Макс, не спи! Ты должен смотреть - это твоё наказание! Если будешь спать, то я буду тебя пихать..."
    Lisa_13 " Max, ¡mantente despierto! Tienes que mirar, ¡este es tu castigo! Si duermes, te empujaré..."

# game/events/lisa.rpy:956
translate spanish lisa_romantic_movie_0_a5e1074f:

    # Max_02 "{m}Я бы тоже с огромным удовольствием попихал в тебя чем-нибудь! А если бы она ещё и уснула со мной в обнимку это было бы...{/m}"
    Max_02 " {m} ¡Me encantaría empujarte algo también! Y si ella también se durmiera conmigo en un abrazo sería... {/ m}"

# game/events/lisa.rpy:957
translate spanish lisa_romantic_movie_0_14665ede:

    # Lisa_10 "Ой-ёй-ёй! У фильма же вроде семейный рейтинг?! Почему они раздеваются?"
    Lisa_10 "Oye ¡¿La película parece tener una calificación familiar?! ¿Por qué se desnudan?"

# game/events/lisa.rpy:958
translate spanish lisa_romantic_movie_0_36bae7b4:

    # Max_03 "А вот это уже будет поинтереснее смотреть! Хороший момент, мне нравится..."
    Max_03 " ¡Pero esto será más interesante de ver! Buen punto, me gusta..."

# game/events/lisa.rpy:959
translate spanish lisa_romantic_movie_0_938027f4:

    # Lisa_11 "Давай промотаем! Мне как-то неудобно... Ого! А что это он там делает ей?!"
    Lisa_11 "¡Vamos a perdernos! Estoy un poco incómoda... ¡Uau! ¡¿Y qué le está haciendo allí?!"

# game/events/lisa.rpy:960
translate spanish lisa_romantic_movie_0_77908404:

    # Max_05 "Я не против, что персонажей в этой сцене решили... хорошенько раскрыть..."
    Max_05 "no me importa que los personajes de esta escena se decidieran... bien revelado..."

# game/events/lisa.rpy:961
translate spanish lisa_romantic_movie_0_f0429dea:

    # Lisa_12 "Чем это ты ноутбук шевелишь? У тебя рука что ли трясётся или..."
    Lisa_12 " ¿Con qué estás moviendo la computadora portátil? ¿Te tiembla la mano o..."

# game/events/lisa.rpy:962
translate spanish lisa_romantic_movie_0_e9d4d46d:

    # Max_07 "Ну... почти."
    Max_07 "Bueno... casi."

# game/events/lisa.rpy:966
translate spanish lisa_romantic_movie_0_f2db6ad8:

    # Lisa_11 "Макс! У тебя встал что ли?"
    Lisa_11 " ¡Max! ¿Se te ha levantado o algo?"

# game/events/lisa.rpy:967
translate spanish lisa_romantic_movie_0_62089fb5:

    # Max_08 "Да, немного. Такой уж фильм ты выбрала!"
    Max_08 " Sí, un poco. ¡Has elegido una película así!"

# game/events/lisa.rpy:968
translate spanish lisa_romantic_movie_0_1ad44cc8:

    # Lisa_13 "Немного?! Это не немного! Какой же ты озабоченный!"
    Lisa_13 " ¿Un poco? ¡No es mucho! ¡Qué preocupado eres!"

# game/events/lisa.rpy:969
translate spanish lisa_romantic_movie_0_e53fb6ea:

    # Max_09 "Вообще-то, так все мужчины реагируют на такое!"
    Max_09 "En realidad, ¡así es como todos los hombres reaccionan a esto!"

# game/events/lisa.rpy:970
translate spanish lisa_romantic_movie_0_1095a0d0:

    # Lisa_12 "Молодец, Макс! Испортил весь просмотр."
    Lisa_12 "¡Bien hecho, Max! arruinaste toda la vista."

# game/events/lisa.rpy:971
translate spanish lisa_romantic_movie_0_f9bf1574:

    # Max_07 "А ты на экран смотри, а не на член."
    Max_07 "Y estás mirando la pantalla, no mi pene."

# game/events/lisa.rpy:972
translate spanish lisa_romantic_movie_0_1574058f:

    # Lisa_10 "Да не могу я смотреть на экран, когда и там и у тебя такое... Всё, я спать!"
    Lisa_10 " Sí, no puedo mirar la pantalla, cuando hay y tienes esto... ¡Eso es, me voy a dormir!"

# game/events/lisa.rpy:974
translate spanish lisa_romantic_movie_0_288ac083:

    # Max_08 "Погоди, но это ведь считается, что я отбыл наказание?"
    Max_08 " Espera, ¿pero se considera que cumplí mi condena?"

# game/events/lisa.rpy:975
translate spanish lisa_romantic_movie_0_3dfedd8c:

    # Lisa_09 "Да ну тебя!"
    Lisa_09 " ¡Vamos!"

# game/events/lisa.rpy:976
translate spanish lisa_romantic_movie_0_85436806:

    # Max_01 "Ладно. Доброй ночи тогда."
    Max_01 "Está bien. Buenas noches entonces."

# game/events/lisa.rpy:993
translate spanish lisa_romantic_movie_r_f3247db8:

    # Lisa_01 "Ну что, Макс, смотрим кино или как?"
    Lisa_01 "Bueno, Max, ¿ver una película o qué?"

# game/events/lisa.rpy:994
translate spanish lisa_romantic_movie_r_a59bad84:

    # Max_01 "Да, смотрим. Сейчас всё подготовлю..."
    Max_01 "sí, veamos. Prepararé todo ahora..."

# game/events/lisa.rpy:995
translate spanish lisa_romantic_movie_r_8ae1d92e:

    # Lisa_02 "А я пока свет выключу."
    Lisa_02 "Y apagaré las luces por ahora."

# game/events/lisa.rpy:1001
translate spanish lisa_romantic_movie_r_8885d6ca:

    # Max_04 "Давай, запрыгивай! Будем досматривать тот фильм?"
    Max_04 "¡Vamos, salta! ¿Vamos a ver esa película?"

# game/events/lisa.rpy:1002
translate spanish lisa_romantic_movie_r_04dfcf7d:

    # Lisa_09 "Да, но промотай тот момент, ну ты понял..."

# game/events/lisa.rpy:1004
translate spanish lisa_romantic_movie_r_c28db2f2:

    # Max_03 "Считай, что уже сделано. Я к отбытию наказания готов!"

# game/events/lisa.rpy:1005
translate spanish lisa_romantic_movie_r_5bd81bcd:

    # Lisa_02 "И смотри чтобы ничего у тебя там не шевелилось больше!"

# game/events/lisa.rpy:1007
translate spanish lisa_romantic_movie_r_16a5711d:

    # Max_04 "Давай, запрыгивай! Ты уже знаешь, что будем смотреть?"

# game/events/lisa.rpy:1008
translate spanish lisa_romantic_movie_r_96f14ddf:

    # Lisa_01 "Нет. Выводи список романтический фильмов и я выберу. Только давай нормальные фильмы, а не как в прошлый раз!"

# game/events/lisa.rpy:1010
translate spanish lisa_romantic_movie_r_64e0ea74:

    # Max_03 "Ты сама выбирала, моё дело маленькое. Вот этот вроде ничего должен быть..."

# game/events/lisa.rpy:1011
translate spanish lisa_romantic_movie_r_598bb99a:

    # Lisa_02 "Ага, давай его. И твоё маленькое дело - это смотреть фильм, мучиться и чтобы у тебя ничего не шевелилось!"

# game/events/lisa.rpy:1021
translate spanish lisa_romantic_movie_r_f037f686:

    # Max_07 "{m}Легко говорить, чтобы ничего не шевелилось! Достаточно просто представить, как Лиза лежит рядом со мной, совсем обнажённая... Ой, лучше не думать!{/m}"

# game/events/lisa.rpy:1022
translate spanish lisa_romantic_movie_r_fc9d7a66:

    # Lisa_03 "Что, Макс, заскучал? Будешь знать, как за мной подглядывать! И не вздумай спать, а то я начну тебя щипать..."

# game/events/lisa.rpy:1023
translate spanish lisa_romantic_movie_r_c8696fac:

    # Max_02 "Так я и ответить могу тем же, если ты не в курсе!"

# game/events/lisa.rpy:1024
translate spanish lisa_romantic_movie_r_8aa097ce:

    # Lisa_13 "Эй! Нет, меня нельзя щипать! Ой, ну вот опять откровенные сцены начались..."

# game/events/lisa.rpy:1025
translate spanish lisa_romantic_movie_r_a517a1f8:

    # Max_04 "{m}Вовремя! А то у меня уже слегка привстал, ведь в голову пришло уже столько пошлых мыслей от того, что Лиза лежит так близко ко мне.{/m}"

# game/events/lisa.rpy:1032
translate spanish lisa_romantic_movie_r_ef4f9485:

    # Lisa_10 "Макс, у тебя опять стоит! Ну сколько можно?"

# game/events/lisa.rpy:1033
translate spanish lisa_romantic_movie_r_8309f28f:

    # Max_01 "Фильмы такие! Что тут поделать..."

# game/events/lisa.rpy:1034
translate spanish lisa_romantic_movie_r_b7875281:

    # Lisa_01 "А я знаю что! Мы будем смотреть что-нибудь такое, на что ты не будешь реагировать так, как сейчас."

# game/events/lisa.rpy:1035
translate spanish lisa_romantic_movie_r_7c8eec32:

    # Max_05 "Например? О, давай боевики?! Или комедии? А ещё лучше - комедийные боевики!"

# game/events/lisa.rpy:1036
translate spanish lisa_romantic_movie_r_7b8935fd:

    # Lisa_02 "Нет, мы будем смотреть ужастики!"

# game/events/lisa.rpy:1037
translate spanish lisa_romantic_movie_r_d996f142:

    # Max_09 "Ужастики?! Ты уверена, что смотреть ужастики в тёмное время суток - это то, что нужно?"

# game/events/lisa.rpy:1038
translate spanish lisa_romantic_movie_r_0a46ff77:

    # Lisa_03 "Что, Макс, струсил?! Это то, что мне и нужно. Наверняка, ты будешь визжать от страха, как маленькая девочка!"

# game/events/lisa.rpy:1039
translate spanish lisa_romantic_movie_r_1ed9f3b6:

    # Max_16 "Не дождёшься! Считай, что вызов я принял! Ещё увидим, кто будет визжать..."

# game/events/lisa.rpy:1040
translate spanish lisa_romantic_movie_r_eeef1b80:

    # Lisa_02 "Да, да... Не удивлюсь, если подглядывания за мной... прекратятся."

# game/events/lisa.rpy:1041
translate spanish lisa_romantic_movie_r_d629e652:

    # Max_09 "У тебя, кстати, есть своя кровать, помнишь?"

# game/events/lisa.rpy:1042
translate spanish lisa_romantic_movie_r_1e0138d1:

    # Lisa_01 "Тогда я пойду спать, а то ты сегодня что-то не в духе."

# game/events/lisa.rpy:1044
translate spanish lisa_romantic_movie_r_cc58a4e6:

    # Max_15 "Да, будь так добра..."

# game/events/lisa.rpy:1050
translate spanish lisa_romantic_movie_r_18f9da30:

    # Lisa_12 "Ну и ты следом сразу возбудился! Это вообще нормально?"

# game/events/lisa.rpy:1051
translate spanish lisa_romantic_movie_r_fe3c40d7:

    # Max_07 "Спроси хоть у кого и ответ будет всегда один - да, это нормальная реакция."

# game/events/lisa.rpy:1052
translate spanish lisa_romantic_movie_r_3327948b:

    # Lisa_10 "Ну и как теперь дальше фильм смотреть, а Макс?"

# game/events/lisa.rpy:1053
translate spanish lisa_romantic_movie_r_7493299f:

    # Max_02 "Как и до этого смотрела, просто с небольшим бонусом."

# game/events/lisa.rpy:1054
translate spanish lisa_romantic_movie_r_f2657b97:

    # Lisa_09 "Ага! Очень такой \"небольшой\" бонус, аж в трусах не умещается... Всё, я спать!"

# game/events/lisa.rpy:1055
translate spanish lisa_romantic_movie_r_56b58bae:

    # Max_07 "Ничего, если я тут досмотрю этот момент, интересно, как закончится?"

# game/events/lisa.rpy:1056
translate spanish lisa_romantic_movie_r_1e238823:

    # Lisa_12 "Нет! Выключай всё! А то это уже не наказание получается."

# game/events/lisa.rpy:1058
translate spanish lisa_romantic_movie_r_79a58dcb:

    # Max_01 "Ладно. Тогда спим..."

# game/events/lisa.rpy:1061
translate spanish lisa_romantic_movie_r_91c5b8d8:

    # Lisa_10 "Макс, может уже хватит меня смущать?! Почему ты такой озабоченный?"

# game/events/lisa.rpy:1062
translate spanish lisa_romantic_movie_r_6a31b0c6:

    # Max_02 "Не понимаю о чём ты говоришь, у меня всё нормально."

# game/events/lisa.rpy:1063
translate spanish lisa_romantic_movie_r_53814219:

    # Lisa_13 "Ага, развалился тут со своим членом на всю кровать и довольный! А это вообще-то наказание!"

# game/events/lisa.rpy:1064
translate spanish lisa_romantic_movie_r_24f4c2ff:

    # Max_07 "Ну а что я сделаю, если они там решили поразвлечься?!"

# game/events/lisa.rpy:1065
translate spanish lisa_romantic_movie_r_754adc77:

    # Lisa_09 "Опять ты весь просмотр испортил! Я пошла спать..."

# game/events/lisa.rpy:1066
translate spanish lisa_romantic_movie_r_d7ff6c8f:

    # Max_04 "Ну давай, а я ещё немного посмотрю..."

# game/events/lisa.rpy:1067
translate spanish lisa_romantic_movie_r_96ee1624:

    # Lisa_10 "Нет уж! Давай выключай! А то лицо у тебя слишком довольное стало."

# game/events/lisa.rpy:1068
translate spanish lisa_romantic_movie_r_95c980ce:

    # Max_03 "И не только лицо..."

# game/events/lisa.rpy:1069
translate spanish lisa_romantic_movie_r_3dfedd8c:

    # Lisa_09 "Да ну тебя!"
    Lisa_09 " ¡Vamos!"

# game/events/lisa.rpy:1071
translate spanish lisa_romantic_movie_r_85436806:

    # Max_01 "Ладно. Доброй ночи тогда."
    Max_01 "Está bien. Buenas noches entonces."

# game/events/lisa.rpy:1089
translate spanish lisa_horor_movie_0_f3247db8:

    # Lisa_01 "Ну что, Макс, смотрим кино или как?"
    Lisa_01 "Bueno, Max, ¿ver una película o qué?"

# game/events/lisa.rpy:1090
translate spanish lisa_horor_movie_0_a59bad84:

    # Max_01 "Да, смотрим. Сейчас всё подготовлю..."
    Max_01 "sí, veamos. Prepararé todo ahora..."

# game/events/lisa.rpy:1091
translate spanish lisa_horor_movie_0_bb19dc25:

    # Lisa_02 "А я пока свет выключу. Тебе уже страшно?"

# game/events/lisa.rpy:1096
translate spanish lisa_horor_movie_0_0302cbc0:

    # Max_07 "Как бы не так! Ты уже знаешь, что будем смотреть?"

# game/events/lisa.rpy:1098
translate spanish lisa_horor_movie_0_46ea63fd:

    # Lisa_03 "Я думала посмотреть все части \"Кошмара на улице Вязов\" или \"Пятницы 13-е\". Мне в школе посоветовали. Но выбирать тебе, ты же будешь бояться." nointeract

# game/events/lisa.rpy:1111
translate spanish lisa_horor_movie_0_d62d1fc8:

    # Lisa_02 "Макс, если тебе будет сильно страшно, то так и скажи! В этом нет ничего такого, мы сразу всё выключим."

# game/events/lisa.rpy:1112
translate spanish lisa_horor_movie_0_d532a320:

    # Max_03 "Мне нечего бояться, с моей стороны стена, так что никто из под кровати на меня не нападёт. Чего не могу сказать о твоём положении, ты сильно рискуешь!"

# game/events/lisa.rpy:1117
translate spanish lisa_horor_movie_0_d3390a12:

    # Lisa_00 "Хорошая попытка, но меня этим не напугаешь, наверно... Вот молчал бы и я об этом сейчас не думала бы!"

# game/events/lisa.rpy:1118
translate spanish lisa_horor_movie_0_a139b038:

    # Max_02 "Но если тебе всё же начнёт казаться, как что-то тянется из темноты к твоей ноге, то сразу скажи. И мы сразу всё выключим!"

# game/events/lisa.rpy:1122
translate spanish lisa_horor_movie_0_793be321:

    # Lisa_09 "Не пугай меня, Макс! И так фильм страшный, так ты тут ещё жути нагоняешь!"

# game/events/lisa.rpy:1123
translate spanish lisa_horor_movie_0_9243c41a:

    # Max_00 "Не бойся, мне тоже страшно!"

# game/events/lisa.rpy:1124
translate spanish lisa_horor_movie_0_7a93c95f:

    # Lisa_10 "Правда? По тебе не скажешь..."

# game/events/lisa.rpy:1125
translate spanish lisa_horor_movie_0_53effe8a:

    # Max_04 "Есть немного, но трусишка у нас ты... Но мне это нравится, очень мило."

# game/events/lisa.rpy:1130
translate spanish lisa_horor_movie_0_6c701e39:

    # Lisa_11 "Ой-ёй-ёй... Зря мы это смотрим! Кажется, я теперь от таких ужасов не смогу заснуть..."

# game/events/lisa.rpy:1131
translate spanish lisa_horor_movie_0_dfc5290b:

    # Max_09 "Ну, ты не одна в комнате, так что бояться нечего. Всех монстров я возьму на себя!"

# game/events/lisa.rpy:1132
translate spanish lisa_horor_movie_0_5bde3f33:

    # Lisa_13 "Макс, это что мне в ногу такое твёрдое упёрлось?!"

# game/events/lisa.rpy:1133
translate spanish lisa_horor_movie_0_d3ed354f:

    # Max_07 "Ноутбук, должно быть."

# game/events/lisa.rpy:1137
translate spanish lisa_horor_movie_0_223fddda:

    # Lisa_12 "Ага, ноутбук, как же! Сейчас-то у тебя от чего встал?!"

# game/events/lisa.rpy:1138
translate spanish lisa_horor_movie_0_a1d4a1fc:

    # Max_08 "Ты ко мне прижалась, вот я и возбудился. Пора бы тебе уже спокойно на это реагировать и не обращать внимание."

# game/events/lisa.rpy:1139
translate spanish lisa_horor_movie_0_bda43f4d:

    # Lisa_09 "Да как тут внимание не обращать, ты же меня своим членом сейчас трогал?!"

# game/events/lisa.rpy:1140
translate spanish lisa_horor_movie_0_2ba42890:

    # Max_09 "Во-первых, не трогал, а ты просто положила на него свою ногу, а во-вторых, ты же к моей ноге своей киской тоже прижалась... Так что всё честно!"

# game/events/lisa.rpy:1145
translate spanish lisa_horor_movie_0_42cba858:

    # Lisa_10 "Ничем таким я к тебе не прижималась! И вообще, я спать пошла... только страшно..."

# game/events/lisa.rpy:1147
translate spanish lisa_horor_movie_0_cd73a907:

    # Max_01 "Ладно. И попку давай береги по пути, а то монстры любят хватать за что-нибудь такое!"

# game/events/lisa.rpy:1148
translate spanish lisa_horor_movie_0_3202fbc2:

    # Lisa_11 "Ой ой ой!"

# game/events/lisa.rpy:1170
translate spanish lisa_horor_movie_r_f3247db8:

    # Lisa_01 "Ну что, Макс, смотрим кино или как?"
    Lisa_01 "Bueno, Max, ¿ver una película o qué?"

# game/events/lisa.rpy:1172
translate spanish lisa_horor_movie_r_766c16b3:

    # Max_01 "Да, смотрим. Сейчас всё подготовлю... Не стесняйся, снимай маечку."

# game/events/lisa.rpy:1173
translate spanish lisa_horor_movie_r_7a0c4061:

    # Lisa_02 "Сейчас сниму, только свет сначала выключу. Тебе уже страшно?"

# game/events/lisa.rpy:1178
translate spanish lisa_horor_movie_r_a59bad84:

    # Max_01 "Да, смотрим. Сейчас всё подготовлю..."
    Max_01 "sí, veamos. Prepararé todo ahora..."

# game/events/lisa.rpy:1179
translate spanish lisa_horor_movie_r_bb19dc25:

    # Lisa_02 "А я пока свет выключу. Тебе уже страшно?"

# game/events/lisa.rpy:1187
translate spanish lisa_horor_movie_r_eeace2b8:

    # Max_07 "Как бы не так! Будем досматривать тот фильм, который тогда смотрели?"

# game/events/lisa.rpy:1189
translate spanish lisa_horor_movie_r_821fd45a:

    # Lisa_00 "Ой, я даже не знаю... Главное, чтобы и тебе было страшно! И желательно, чтобы больше, чем мне." nointeract

# game/events/lisa.rpy:1222
translate spanish lisa_horor_movie_r_40f992d8:

    # Max_07 "Как бы не так! Будем смотреть следующий фильм в той серии, которую начали?"

# game/events/lisa.rpy:1225
translate spanish lisa_horor_movie_r_821fd45a_1:

    # Lisa_00 "Ой, я даже не знаю... Главное, чтобы и тебе было страшно! И желательно, чтобы больше, чем мне." nointeract

# game/events/lisa.rpy:1275
translate spanish lisa_horor_movie_r_10373a9d:

    # Lisa_09 "Может хоть для приличия испугаешься? А то иначе я уже не знаю, как тебя наказать, разве что маме тебя сдать..."

# game/events/lisa.rpy:1276
translate spanish lisa_horor_movie_r_5ed34e7c:

    # Max_14 "Я боюсь! Теперь стало намного страшнее после твоих слов."

# game/events/lisa.rpy:1280
translate spanish lisa_horor_movie_r_6f2e38dc:

    # Lisa_10 "Вот и хорошо, а то я не хочу одна бояться. Ну вот что они делают?! Это же точно ничем хорошим не закончится!"

# game/events/lisa.rpy:1281
translate spanish lisa_horor_movie_r_5a7b0ed9:

    # Max_02 "{m}Хорошо, что в ужастиках куча тупых персонажей, потому что это гарантирует мне крепкие объятия от Лизы! Главное стараться не думать, какими прелестями она ко мне прижимается.{/m}"

# game/events/lisa.rpy:1288
translate spanish lisa_horor_movie_r_6c701e39:

    # Lisa_11 "Ой-ёй-ёй... Зря мы это смотрим! Кажется, я теперь от таких ужасов не смогу заснуть..."

# game/events/lisa.rpy:1291
translate spanish lisa_horor_movie_r_03a8041a:

    # Max_10 "{m}Только бы у меня не встал! У меня тут полный эффект погружения... Ладно в ужастике сиськи голые периодически мелькают, а вот голая грудь моей сестрёнки, которой она слегка трётся о меня, вот это проблема... Как тут сдерживаться?{/m}" nointeract

# game/events/lisa.rpy:1293
translate spanish lisa_horor_movie_r_d21ad010:

    # Max_10 "{m}Только бы у меня не встал! Ещё периодически сиськи голые в ужастике мелькают... Как тут сдерживаться?{/m}" nointeract

# game/events/lisa.rpy:1298
translate spanish lisa_horor_movie_r_11d3ed3b:

    # Lisa_13 "[norestrain!t]Макс, мне кажется или у меня под ногой сейчас что-то увеличивается?"

# game/events/lisa.rpy:1304
translate spanish lisa_horor_movie_r_12b43dd4:

    # Lisa_09 "[restrain!t]Макс, я уже спать хочу. Давай закругляться. Да и набоялась я уже слишком..."

# game/events/lisa.rpy:1307
translate spanish lisa_horor_movie_r_a3b4a2f9:

    # Lisa_09 "[restrain!t]Наконец-то фильм заканчивается, а то я набоялась уже сполна..."

# game/events/lisa.rpy:1310
translate spanish lisa_horor_movie_r_e06d84d1:

    # Max_04 "Ага, я тоже. Было страшно, но я рад, что ты была рядом. Это приятно."

# game/events/lisa.rpy:1316
translate spanish lisa_horor_movie_r_e8dd9914:

    # Lisa_10 "Мне только страшно до своей кровати идти теперь..."

# game/events/lisa.rpy:1317
translate spanish lisa_horor_movie_r_7656a27c:

    # Max_03 "Так не иди. Спи со мной. Я очень даже не против!"

# game/events/lisa.rpy:1319
translate spanish lisa_horor_movie_r_2ccbf395:

    # Lisa_05 "Чтобы со мной рядом кое-что шевелилось? Так я точно не усну. Мне нужно как-то храбрости набраться..." nointeract

# game/events/lisa.rpy:1330
translate spanish lisa_horor_movie_r_a6cf8168:

    # Max_05 "{m}Нежный поцелуй с сестрёнкой перед сном точно отвлечёт её от всяких страхов. Целуя её, вообще забываешь о том, что там было перед этим... Лишь её сочные губки и нежная грудь, которой она касается меня...{/m}"

# game/events/lisa.rpy:1335
translate spanish lisa_horor_movie_r_12129bd6:

    # Max_05 "{m}Нежный поцелуй с сестрёнкой перед сном точно отвлечёт её от всяких страхов. Целуя её, вообще забываешь о том, что там было перед этим... Лишь её сочные губки...{/m}"

# game/events/lisa.rpy:1342
translate spanish lisa_horor_movie_r_5aae11b2:

    # Lisa_02 "Да, так уже совсем не страшно. Я пойду... Спокойной ночи, Макс."

# game/events/lisa.rpy:1343
translate spanish lisa_horor_movie_r_5c2cac39:

    # Max_01 "Ага. Приятных снов."

# game/events/lisa.rpy:1353
translate spanish lisa_horor_movie_r_62242552:

    # Lisa_13 "Ну ага, просто иди! А вдруг меня что-то схватит?!"

# game/events/lisa.rpy:1354
translate spanish lisa_horor_movie_r_4217f727:

    # Max_07 "У нас в комнате нет никаких монстров! Если конечно не считать того, что у меня в трусах."

# game/events/lisa.rpy:1358
translate spanish lisa_horor_movie_r_08782138:

    # Lisa_01 "Ой, с тобой и правда страшно спать будет! Я пошла к себе..."

# game/events/lisa.rpy:1359
translate spanish lisa_horor_movie_r_1dc372f8:

    # Max_01 "Ага. Спокойной ночи."

# game/events/lisa.rpy:1363
translate spanish lisa_horor_movie_r_26ebf379:

    # Lisa_13 "Макс, мне кажется или у меня под ногой сейчас что-то увеличивается?"

# game/events/lisa.rpy:1367
translate spanish lisa_horor_movie_r_not_restrain_e9260b73:

    # Max_07 "Однозначно кажется..."

# game/events/lisa.rpy:1373
translate spanish lisa_horor_movie_r_not_restrain_04d9f431:

    # Lisa_12 "Ага, кажется, как же! Опять ты возбудился... Хотя, ничего удивительного, я была почти уверена, что так и будет."

# game/events/lisa.rpy:1374
translate spanish lisa_horor_movie_r_not_restrain_c4107b2d:

    # Max_09 "Ну встал и встал, подумаешь. Спрячу под ноутбуком."

# game/events/lisa.rpy:1376
translate spanish lisa_horor_movie_r_not_restrain_afe91d1a:

    # Lisa_12 "Ага, кажется, как же! Опять ты возбудился... Я же твоя сестра, тебе стыдно должно быть!"

# game/events/lisa.rpy:1377
translate spanish lisa_horor_movie_r_not_restrain_5876b4d3:

    # Max_09 "Стыдно?! Ну встал и встал, подумаешь."

# game/events/lisa.rpy:1382
translate spanish lisa_horor_movie_r_not_restrain_8ab024b6:

    # Lisa_10 "Хватит уже похабно думать обо мне! Я ушла спать... Ой! Как страшно..."

# game/events/lisa.rpy:1383
translate spanish lisa_horor_movie_r_not_restrain_75e90268:

    # Max_04 "Проводить тебя до кровати?"

# game/events/lisa.rpy:1384
translate spanish lisa_horor_movie_r_not_restrain_f26114cd:

    # Lisa_13 "Сама справлюсь! Ой ой ой!"
    new ""

translate spanish strings:

    # game/events/lisa.rpy:44
    old "Это я, Макс!"
    new ""

    # game/events/lisa.rpy:47
    old "Можно я войду? Мне очень нужно..."
    new ""

    # game/events/lisa.rpy:47
    old "Хорошо, я подожду"
    new ""

    # game/events/lisa.rpy:50
    old "Ладно, ладно..."
    new ""

    # game/events/lisa.rpy:292
    old "Можно войти на секунду? Я только ноутбук возьму..."
    new ""

    # game/events/lisa.rpy:298
    old "{i}всё равно забрать ноутбук{/i}"
    new ""

    # game/events/lisa.rpy:362
    old "{i}уйти с ноутбуком на веранду{/i}"
    new ""

    # game/events/lisa.rpy:371
    old "{i}забрать ноутбук{/i}"
    new ""

    # game/events/lisa.rpy:403
    old "Да чего я там не видел..."
    new ""

    # game/events/lisa.rpy:403
    old "А при мне ты этого сделать не можешь?"
    new ""

    # game/events/lisa.rpy:403
    old "Хорошо, только ноутбук заберу..."
    new "Está bien, solo tomaré la computadora portátil..."

    # game/events/lisa.rpy:406
    old "Не буду. Честное слово!"
    new "No lo haré. ¡Ni una palabra al respecto!"

    # game/events/lisa.rpy:445
    old "{i}отвернуться{/i}"
    new "{i}alejarse{/i}"

    # game/events/lisa.rpy:445
    old "{i}подождать за дверью, пока Лиза переоденется{/i}"
    new "{i}esperar afuera de la puerta a que Lisa se cambie de vestido{/i}"

    # game/events/lisa.rpy:455
    old "{i}не подглядывать{/i}"
    new "{i}no espiar{/i}"

    # game/events/lisa.rpy:455
    old "{i}попробовать подглядеть{/i}"
    new "{i}tratar de espiar{/i}"

    # game/events/lisa.rpy:516
    old "А у тебя сиськи подросли!"
    new "¡Sus tetas han crecido!"

    # game/events/lisa.rpy:516
    old "Отличный зад, я тебе скажу!"
    new "¡Gran culo, te lo diré!"

    # game/events/lisa.rpy:516
    old "А если я хочу на это посмотреть?"
    new "¿Qué pasa si quiero verlo?"

    # game/events/lisa.rpy:516
    old "Извини, я не знал... Красиво смотришься!"
    new "Lo siento, no lo sabía... ¡Te ves muy bien!"

    # game/events/lisa.rpy:530
    old "У тебя красивая грудь, Лиза. Не стесняйся..."
    new "Tienes senos hermosos, Lisa. No seas tímida..."

    # game/events/lisa.rpy:530
    old "Какая соблазнительная попка у тебя..."
    new "Que culo tan seductor tienes..."

    # game/events/lisa.rpy:530
    old "А где же волшебное слово?"
    new "¿Y dónde está la palabra mágica?"

    # game/events/lisa.rpy:544
    old "У тебя красивая грудь, Лиза. Не прячь её..."
    new "Tienes senos hermosos, Lisa. No los escondas..."

    # game/events/lisa.rpy:544
    old "Эх, не повезло! Я надеялся, что ты будешь раздета..."
    new "¡Oh, sin suerte! Esperaba que te desnudaras..."

    # game/events/lisa.rpy:544
    old "Кто-то у нас тут трусики не успел надеть..."
    new "Alguien aquí no tuvo tiempo de ponerse bragas..."

    # game/events/lisa.rpy:544
    old "Извини, я не знал... Ты такая очаровательная!"
    new "Lo siento, no lo sabía... ¡Eres tan encantador!"

    # game/events/lisa.rpy:596
    old "Подумаешь, сестрёнку полуголую увидел..."
    new "Piénsalo, vi a mi hermana semidesnuda..."

    # game/events/lisa.rpy:596
    old "Да ладно тебе, можешь меня не стесняться!"
    new "¡Vamos, no te avergüences de mí!"

    # game/events/lisa.rpy:596
    old "Извини, не удержался... Хорошо выглядишь!"
    new "Lo siento, no pude resistir... ¡Te ves bien!"

    # game/events/lisa.rpy:596
    old "А ты представь, что меня тут на самом деле нет!"
    new "¡Y te imaginas que realmente no estoy aquí!"

    # game/events/lisa.rpy:596
    old "Извини, не удержался... Ты очень красивая!"
    new "Lo siento, no pude resistirme... ¡Eres muy bella!"

    # game/events/lisa.rpy:824
    old "Это я, Макс! Можно войти?"
    new "¡Soy yo, Max! ¿Puedo entrar?"

    # game/events/lisa.rpy:873
    old "Извини, дверь была открыта и я подумал..."
    new "Lo siento, la puerta estaba abierta y pensé..."

    # game/events/lisa.rpy:873
    old "А ты симпатичная..."
    new "Y eres bonita..."

    # game/events/lisa.rpy:873
    old "И что такого? Сестра стесняется брата?"
    new "¿Y qué? ¿Mi hermana está avergonzada por su hermano?"

    # game/events/lisa.rpy:1097
    old "{i}смотреть \"Кошмар на улице Вязов\"{/i}"
    new "{i}ver \"A Nightmare on Elm Street\"{/i}"

    # game/events/lisa.rpy:1097
    old "{i}смотреть \"Пятница 13-е\"{/i}"
    new "{i}ver \"Viernes 13\"{/i}"

    # game/events/lisa.rpy:1097
    old "{i}смотреть \"Крик\"{/i}"
    new "{i}ver \"Scream\"{/i}"

    # game/events/lisa.rpy:1188
    old "Тогда досматриваем... (продолжаем смотреть \"Кошмар на улице Вязов\")"
    new "Luego inspeccionamos... (continúa viendo \"A Nightmare on Elm Street\")"

    # game/events/lisa.rpy:1188
    old "Тогда досматриваем... (продолжаем смотреть \"Пятница 13-е\")"
    new "Luego inspeccionamos... (seguir viendo \"Viernes 13\")"

    # game/events/lisa.rpy:1188
    old "Тогда досматриваем... (продолжаем смотреть \"Крик\")"
    new "Luego inspeccionamos... (sigue viendo \"Scream\")"

    # game/events/lisa.rpy:1224
    old "Тогда смотрим дальше... (продолжаем смотреть серию фильмов \"Кошмар на улице Вязов\")"
    new "Entonces miramos más lejos... (continuar viendo la serie de películas \"Pesadilla en Elm Street\")"

    # game/events/lisa.rpy:1224
    old "Тогда смотрим дальше... (продолжаем смотреть серию фильмов \"Пятница 13-е\")"
    new "Entonces miramos más lejos...(seguir viendo la serie de películas \"Viernes 13\")"

    # game/events/lisa.rpy:1224
    old "Тогда смотрим дальше... (продолжаем смотреть серию фильмов \"Крик\")"
    new "Entonces miramos más lejos...(seguir viendo la serie de películas \"Viernes 13 \")"

    # game/events/lisa.rpy:1294
    old "{i}да пофиг!{/i}"
    new "{i}no importa!{/i}"

    # game/events/lisa.rpy:1318
    old "Просто иди и всё..."
    new "Solo ve y eso es todo..."

