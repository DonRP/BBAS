# TODO: Translation updated at 2020-10-14 09:08

# game/events/lisa_cam.rpy:8
translate english cam0_lisa_sleep_night_cab47b53:

    # Max_01 "Лиза сладко спит..."
    Max_01 "Lisa is sleeping soundly..."

# game/events/lisa_cam.rpy:17
translate english cam0_lisa_sleep_morning_36c03e6c:

    # Max_01 "Лиза ещё спит..."
    Max_01 "Lisa is still asleep..."

# game/events/lisa_cam.rpy:26
translate english cam0_lisa_shower_cf65001d:

    # Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"
    Max_09 "I can't see anything through this camera. Может посмотреть через другую?"

# game/events/lisa_cam.rpy:28
translate english cam0_lisa_shower_99a12737:

    # Max_09 "Лизы не видно через эту камеру..."
    Max_09 "I can't see Lisa through this camera..."

# game/events/lisa_cam.rpy:36
translate english cam0_lisa_shower_30b79938:

    # Max_04 "Младшая сестрёнка принимает душ... Прекрасная Лиза - прекрасное утро!"
    Max_04 "Lisa's taking a shower... Прекрасная Лиза - прекрасное утро!"

# game/events/lisa_cam.rpy:65
translate english cam1_lisa_shower_d17010c3:

    # Max_03 "Лиза, прежде чем принять душ, любуется собой перед зеркалом. И мы этим со зрителями тоже полюбуемся..."
    Max_03 "She's checking herself out in the mirror. И мы этим со зрителями тоже полюбуемся..."

# game/events/lisa_cam.rpy:59
translate english cam1_lisa_shower_cf65001d:

    # Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"
    Max_09 "I can't see Lisa through this camera. Может посмотреть через другую?"

# game/events/lisa_cam.rpy:73
translate english cam0_lisa_read_b78fc025:

    # Max_07 "Люблю смотреть, как Лиза читает. Вернее, люблю позы, в которых она читает..."
    Max_07 "I love watching Lisa read. Вернее, люблю позы, в которых она читает..."

# game/events/lisa_cam.rpy:104
translate english cam0_lisa_dressed_school_f7db1942:

    # Max_09 "Ничего интересного я здесь уже не увижу, Лиза полностью оделась."
    Max_09 "Nothing interesting here.

# game/events/lisa_cam.rpy:115
translate english cam0_lisa_dressed_school_e79b2255:

    # Max_07 "Отлично! Лиза наряжается, чтобы отправиться в школу..." nointeract
    Max_07 "Perfect! Лиза наряжается, чтобы отправиться в школу..." nointeract

# game/events/lisa_cam.rpy:126
translate english cam0_lisa_dressed_school_e61ca9e9:

    # Max_02 "Ухх! Сейчас она такая голенькая и милая..." nointeract
    Max_02 "Yes! Сейчас она такая голенькая и милая..." nointeract

# game/events/lisa_cam.rpy:162
translate english cam0_lisa_dressed_school_e16161a7:

    # Max_04 "Как классно, что моя сестрёнка - такая соблазнительная школьница. Уверен, зрителям это нравится!"
    Max_04 "My little sister is such a naughty girl beneath that uniform. Уверен, зрителям это нравится!"

# game/events/lisa_cam.rpy:140
translate english cam0_lisa_dressed_shop_f7db1942:

    # Max_09 "Ничего интересного я здесь уже не увижу, Лиза полностью оделась."
    Max_09 "I can't see Lisa through this camera.

# game/events/lisa_cam.rpy:151
translate english cam0_lisa_dressed_shop_2978876d:

    # Max_07 "Отлично! Лиза наряжается, чтобы отправиться на шопинг..." nointeract
    Max_02 "Yes! Лиза наряжается, чтобы отправиться на шопинг..." nointeract

# game/events/lisa_cam.rpy:173
translate english cam0_lisa_dressed_shop_e61ca9e9:

    # Max_02 "Ухх! Сейчас она такая голенькая и милая..." nointeract
    Max_09 "Nothing interesting. Сейчас она такая голенькая и милая..." nointeract

# game/events/lisa_cam.rpy:187
translate english cam0_lisa_dressed_shop_639be09f:

    # Max_04 "Повезло мне с сестрёнкой! Обворожительна в любой одежде и ещё больше - без неё..."
    Max_04 "Повезло мне с сестрёнкой! Обворожительна в любой одежде и ещё больше - без неё..."

# game/events/lisa_cam.rpy:202
translate english cam0_lisa_sun_fafb408a:

    # Max_01 "Лиза загорает и радует этим моих зрителей! И меня, конечно же..."
    Max_01 "Lisa sunbathing makes a great show for the audience! И меня, конечно же..."

# game/events/lisa_cam.rpy:195
translate english cam1_lisa_sun_ce238a73:

    # Max_09 "Через эту камеру ничего не видно... Может посмотреть через другую?"
    Max_09 "I can't see Lisa through this camera. Может посмотреть через другую?"

# game/events/lisa_cam.rpy:212
translate english cam0_lisa_swim_39c0bef3:

    # Max_09 "Ничего толком не видно... Стоит взглянуть через другую камеру..."
    Max_09 "I can't see anything. Стоит взглянуть через другую камеру..."

# game/events/lisa_cam.rpy:210
translate english cam0_lisa_swim_06c19036:

    # Max_09 "Ничего не разглядеть... Нужно установить камеру, которая охватила бы весь бассейн..."
    Max_09 "I can't really see anything. Нужно установить камеру, которая охватила бы весь бассейн..."

# game/events/lisa_cam.rpy:239
translate english cam1_lisa_swim_0cc13309:

    # Max_01 "Приятно наблюдать за младшей сестрёнкой у водички..."
    Max_01 "Lisa seems to enjoy using the pool..."

# game/events/lisa_cam.rpy:220
translate english cam0_lisa_dishes_0b71076a:

    # Max_01 "Лиза моет посуду. А ведь я мог бы ей помочь..."
    Max_01 "Lisa's doing the dishes. А ведь я мог бы ей помочь..."

# game/events/lisa_cam.rpy:256
translate english cam0_lisa_phone_14f355ab:

    # Max_01 "Сестрёнка бездельничает и залипла в свой телефон. Но лежит красиво..."
    Max_01 "Lisa's only playing on her phone, but she's lying there beautifully..." Но лежит красиво..."

# game/events/lisa_cam.rpy:269
translate english cam0_lisa_bath_b32c78c2:

    # Max_01 "Лиза почти набрала воду, хотя я смотрю на кое-что другое..."
    Max_04 "Lisa's body looks amazing when it's wet!"

# game/events/lisa_cam.rpy:249
translate english cam0_lisa_bath_4e37cfa9:

    # Max_04 "Эх, Лиза... Не вытирайся! Ты мокренькая тоже обалденная..."
    Max_07 "Lisa's getting ready to go shopping..." nointeract Не вытирайся! Ты мокренькая тоже обалденная..."

# game/events/lisa_cam.rpy:277
translate english cam0_lisa_bath_8714e3c3:

    # Max_05 "Давай, сестрёнка, не стесняйся показать как можно больше всего интересного..."
    Max_05 "Давай, сестрёнка, не стесняйся показать как можно больше всего интересного..."

# game/events/lisa_cam.rpy:262
translate english cam1_lisa_bath_cf65001d:

    # Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"
    Max_09 "Лизы не видно через эту камеру... Может посмотреть через другую?"

# game/events/lisa_cam.rpy:229
translate english cam0_lisa_homework_8bf186e7:

    # Max_01 "Лиза учит уроки. Может, стоило ей помочь?!"
    Max_01 "Lisa's doing her homeowork. Может, стоило ей помочь?!"

